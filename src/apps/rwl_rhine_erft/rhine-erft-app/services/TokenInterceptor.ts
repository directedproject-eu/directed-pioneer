...
const PROTECTED_GEOSERVER_BASE_URL = new URL("https://directed.dev.52north.org/secure/geoserver/");
...
export class TokenInterceptor implements Interceptor {
    ...
    beforeRequest({ target, options }: BeforeRequestParams): void {
        ...
        const isProtectedTarget =
            matchesProtocol &&
            target.hostname === PROTECTED_GEOSERVER_BASE_URL.hostname &&
            target.pathname.startsWith(PROTECTED_GEOSERVER_BASE_URL.pathname);
